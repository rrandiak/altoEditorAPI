...
public class AltoOcrService {
    ...
    public AltoOcrService(XmlMapper xmlMapper, XmlLSResolver resolver) throws SAXException {
        this.xmlMapper = xmlMapper;
        this.schemas = loadSchemas(resolver);
    }

    /**
     * Parses ALTO XML and extracts OCR text.
     *
     * @param altoXml the ALTO XML content
     * @return extracted text
     */
    public String convertAltoToOcr(String altoXml) {
        try {
            JsonNode root = xmlMapper.readTree(altoXml);
    ...
    private static List<Schema> loadSchemas(XmlLSResolver resolver) throws SAXException {
        ...
        for (String path : ALTO_SCHEMA_PATHS) {
            result.add(factory.newSchema(
                    AltoOcrService.class.getResource(path)));
        }
